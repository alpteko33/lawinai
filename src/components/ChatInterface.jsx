...
Hangi konuda yardım istiyorsunuz?`
  ...
  return (
    <div className="flex-1 flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        {messages.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-2xl font-bold text-white mb-3 tracking-tight">Gemini AI Hukuki Asistanınız</h3>
            ...
            {files.length === 0 ? (
              <div className="mb-10">
                <h4 className="text-xl font-semibold text-white mb-6">Hızlı Başlangıç</h4>
                <button
              ...
              <div className="mb-10">
                <h4 className="text-xl font-semibold text-white mb-6">Hızlı Komutlar</h4>
                <div className="flex flex-wrap justify-center gap-3">
                  {quickPrompts.slice(0, 3).map((prompt, index) => (
                    <button
                      key={index}
                      onClick={() => handleQuickPrompt(prompt)}
                      className="bg-gray-700 hover:bg-gray-600 text-white px-6 py-3 rounded-lg font-medium transition-all duration-200 hover:scale-105"
                    >
                      {prompt}
                    </button>
                  ))}
            ...
            {/* Petition Types */}
            <div>
              <h4 className="text-xl font-semibold text-white mb-6">Dilekçe Hazırla</h4>
              <div className="grid grid-cols-3 gap-3 max-w-3xl mx-auto">
                {petitionTypes.slice(0, 6).map((type, index) => (
                  <button
                    key={index}
                    onClick={() => handlePetitionRequest(type)}
                    className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-3 rounded-lg font-medium transition-all duration-200 hover:scale-105"
                  >
                    {type}
                  </button>
                ))}
      ...
      {/* Input Area */}
      <div className="border-t border-gray-700/50 p-6 bg-gradient-to-r from-gray-900/50 to-gray-800/30">
        <div className="flex space-x-4">
          <div className="flex-1">
            <textarea
              ...
              className="w-full resize-none bg-gray-800 border border-gray-600 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50 transition-all duration-200"
              rows={3}
              disabled={isLoading}
            />
          </div>
          <button
            ...
            disabled={!inputValue.trim() || isLoading}
                          className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-200 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed self-end"
          >
            ...
            <div className="flex flex-wrap gap-2">
              {quickPrompts.map((prompt, index) => (
                <button
                  key={index}
                  onClick={() => handleQuickPrompt(prompt)}
                  className="text-sm text-gray-400 hover:text-white bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded-lg transition-all duration-200 hover:scale-105"
                >
                  {prompt}
                </button>
              ))}
